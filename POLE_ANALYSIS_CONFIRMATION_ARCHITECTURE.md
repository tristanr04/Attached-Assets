...
The current persistence checkpoint adds typed Drizzle declarations and an explicit repeat-safe SQL migration/rollback for profiles, company type catalogs, protected verified reference photos, immutable versioned runs, ranked candidates, field proposals, and foreman decisions. It is intentionally **not applied**: this repository previously used direct schema push and has no isolated PostgreSQL migration harness. Production execution must wait for a backup, seeded forward/repeat/rollback rehearsal, and deployment approval.
